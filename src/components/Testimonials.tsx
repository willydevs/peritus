...
const testimonials = [
  {
    ...
    result: "Economizou R$ 57.600/ano",
    stars: 5,
  ...
  },
  {
    ...
    segment: "Saúde",
    result: "Economizou R$ 60.000/ano",
    stars: 5,
  ...
  },
  {
    ...
    result: "4 escritórios anteriores trocados",
    stars: 5,
  ...
  },
  {
    ...
    image: null,
    segment: "Educação",
    result: "Empresa aberta em 7 dias",
    stars: 5,
  ...
  },
  {
    ...
    image: null,
    segment: "Logística",
    result: "CNPJ regularizado em 30 dias",
    stars: 5,
...
const segmentColors: Record<string, string> = {
  "Distribuição":    "bg-blue-900/30 text-blue-400 border-blue-800/30",
  "Saúde":           "bg-green-900/30 text-green-400 border-green-800/30",
  "Construção Civil":"bg-yellow-900/30 text-yellow-400 border-yellow-800/30",
  "Educação":        "bg-purple-900/30 text-purple-400 border-purple-800/30",
  "Logística":       "bg-orange-900/30 text-orange-400 border-orange-800/30",
  "Estética & Beleza":"bg-pink-900/30 text-pink-400 border-pink-800/30",
};
          ...
          <div className="embla" ref={emblaRef}>
            <div className="embla__container" style={{ display: "flex", gap: "1.5rem" }}>
              {testimonials.map((t, i) => (
                <div
                  key={i}
                  className="embla__slide px-1 md:basis-1/2 lg:basis-1/3"
                  style={{ minWidth: 0 }}
                >
                  <div className="h-full p-7 bg-[#111] border border-white/5 rounded-sm relative overflow-hidden group hover:border-[#C8102E]/30 transition-all duration-300 flex flex-col">
                    {/* Top accent line */}
                    <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[#C8102E] to-transparent opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden />

                    {/* Segment tag + result badge */}
                    <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
                      <span
                        className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${
                          segmentColors[t.segment] ?? "bg-white/5 text-gray-400 border-white/10"
                        }`}
                      >
                        {t.segment}
                      </span>
                      <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-[#C8102E] text-white">
                        {t.result}
                      </span>
                    </div>

                    {/* Stars */}
                    <div className="flex gap-0.5 mb-3" role="img" aria-label={`${t.stars} estrelas`}>
                    ...
                    {/* Quote mark */}
                    <div
                      className="text-4xl text-[#C8102E]/30 font-black leading-none mb-2 select-none"
                      style={{ fontFamily: "var(--font-playfair)" }}
                      aria-hidden
                    >
                      "
                    ...
                    {/* Text */}
                    <p
                      className="text-gray-400 text-sm leading-relaxed flex-1 mb-6"
                      style={{ fontFamily: "var(--font-inter)" }}
                    >
                      &ldquo;{t.text}&rdquo;
                    </p>
                    ...
                    <div className="flex items-center gap-3 pt-5 border-t border-white/5">
                      <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-[#C8102E] flex items-center justify-center">
                        {t.image ? (
                          ...
                          <span className="text-white text-sm font-bold" aria-label={t.name}>
                            {t.avatar}
                          </span>
                        )}
                      </div>
